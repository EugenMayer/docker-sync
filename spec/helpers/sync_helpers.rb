RSpec::Matchers.define :be_in_sync_with do |container_dir|
  CONTAINER_NAME_MISSING = 'Container name is missing. Please chain `.in_container(container_name)` to define it.'.freeze

  match do |host_dir|
    raise CONTAINER_NAME_MISSING if @container_name.nil? || @container_name.empty?
